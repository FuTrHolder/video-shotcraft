...
- 字标逐字 letterpress：scale 从大压到 1 + blur→0，字底琥珀 glint 短划闪过
- kicker 副标打字机逐字符出现，琥珀块光标周期闪烁
...
| 字标逐字 | 第 i 字 delay=10+i·3、12f；scale 1.6→1（origin center bottom）+ blur 6px→0 | "opacity+位移/scale+blur→0"是全片入场三件套定式（findings 附录），缺 blur 会显得硬 |
...
## 声音
品牌落定钉 transition-soft（源项目 f12），brand→dashboard 运镜钉 whoosh-fast（f78）。SFX 逐拍钉帧、声明式表管理（S2）；音色走电影系词汇，不用游戏 UI tap（S1）。

## 已知坑
- hold 要给的是 wordmark 落定时刻，不是普通字卡/内容卡（R1）——源项目把 hold 加给 card1 后 8 分钟内 revert（76ff0a8a→3d3f364d→dbb1ba87）；指代含糊的"停留"反馈先确认对象再动手（P3）
