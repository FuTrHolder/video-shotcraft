...
## 意图
这是全库"最贵的知识"：用户三轮追打文字模糊、agent 四连击 35 分钟才找到根因的技法。凡是把 UI 截图放进 3D 变换再放大的镜头，迟早撞上它。
...
| DoF | 顶部渐变带 backdropFilter blur，focusY 240、strength 峰值 9 后归零 | DoF 调焦平面治不了糊（381c18bf 实证无效）——它只做氛围，别拿它救清晰度 |

## 声音
技法卡，无声音段。若镜头因此换纹理/改坐标导致时间线变动，SFX 全表重对（S3）。

## 已知坑
- 排查顺序是关键（Q2）：文字糊 → 先查纹理源分辨率链路（截图倍率→栅格化路径→缩放方式），最后才考虑相机——源项目先调 DoF（381c18bf 治标无效）→ 4x 元素截图（ab65cb22 仍糊）→ 原生尺寸栅格化（2401a255）→ CSS zoom 推广（be01ec2c 根治），四连击的前两击都打偏了
- 验证方式：逐帧截图放大看文字边缘有无像素方块（P1 的像素级自检；源项目为此写了 crop.mjs 裁剪放大工具）——肉眼看缩略图发现不了
