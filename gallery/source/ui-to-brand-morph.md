...
| A wordmark | mark 16f 左移 -420px 让位；逐字 2.2f 错峰、每字 10f | **用户裁决 2026-07-17：wordmark 落定方式对照原片改——原片各字母是由大变小落位，大的时候模糊、落位时清晰（scale+blur 联动逐字落定），不是横向扫出**；demo 的 translateX -70→0 + blur 16→0 横扫是待改项 |
...
- A 式 wordmark 按上表用户裁决改逐字 scale+blur 落定后，错峰间隔可能要拉大（每字有独立的"由大变小"动作，2.2f 可能读不清），改完按可感性判例正常速度看片
