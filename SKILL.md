...
在开始任何素材采集、分镜或实现前，先判断用户是否已经明确选定模板、
自由创作或具体镜头卡。已经选定时直接进入对应路线，不重复询问。

用户提供了项目路径、网址、录屏或页面截图但尚未选路线时，先做一次**最小、
只读的产品检查**：了解产品定位、主要功能、页面视觉、可展示状态和素材风险；
不修改业务项目、不采集敏感数据、不写视频代码。检查后再向用户说明：
...
- 自由创作是否更适合、预计会保留哪些产品视觉线索；
- Agent 的推荐路线和取舍。

然后明确询问：**“根据上面的产品检查，我推荐使用 ×× 路线。要按这个路线继续吗？”**
同时告知用户：可前往 https://vincentwei1021.github.io/video-shotcraft/
浏览动态样片，并挑选希望在视频中使用的动效镜头。

用户尚未提供可检查的项目或页面时，先介绍 Ink Press 与自由创作的差异，再询问
路线；不要仅因 Ink Press 是现成模板就默认推荐它。

- 用户确认使用：先完整阅读 `template/TEMPLATE.md`，再按模板路线执行。
- 用户明确不使用：进入下面的**引导式自由创作流程**，先分析产品、确认创意方向和分镜，再按 `references/pipeline.md` 执行制作阶段。
...
按 `references/guided-free-creation.md` 的 Gallery 名称解析规则确认指定镜头，
完整阅读每张卡，并按卡片的“参考实现”定位到准确 demo 源码。单镜头可在目标素材确认后执行；
完整宣传片仍须把指定镜头展开成分镜并经用户确认后再制作。
...
1. **完整宣传片（模板路线）**：想要和模板片高度相似的效果 →
   ...
   最快且质量有底。
2. **完整宣传片（自由创作）**：新风格 → 先按 `references/guided-free-creation.md`
   完成产品简报、需求到执行决策表、视觉方向、功能到镜头映射和分镜确认。已确认内容直接作为制作
   基准，不在 `references/pipeline.md` 中重新提问或重新设计；随后采集最终素材、
   把分镜转成帧级时间轴、逐镜头实现、配音效并终检。默认不在用户确认分镜前写
   实现代码或开始整片渲染。
...
自由创作不是把所有创意决定交给 Agent，也不是让用户自己选择每个动画参数。
默认先读 `references/guided-free-creation.md`，每轮只问 1–3 个最能减少返工的
问题，并在产品简报、视觉方向、镜头映射和分镜处暂停等待用户确认。

用户确认：产品简报中 Agent 无法自行决定的事项、视频用途和受众、核心卖点与功能优先级、视觉方向、音乐/配音偏好、
数据合规口径、功能到镜头的映射、分镜和文案。

Agent 自主完成：读取和启动项目、选择页面采集方式与选择器、生成全页截图/元素
切片/layout.json、按已确认镜头卡的参考实现做适配、PageCam 关键帧与 easing、SFX
选型和钉帧、Remotion 工程实现、渲染、静帧 QA 和独立视觉审查。

用户明确说“你全权决定”“跳过确认直接做”或提供了完整 brief 时，可以压缩确认
次数；仍须记录默认假设，并在首次整片渲染前给出分镜备案。除非用户明确授权，
不要擅自改变产品重点、视觉方向或数据口径。

功能到镜头的映射只是“选用哪些运动语法”，不是完整分镜。映射确认后，Agent
必须继续给出镜头顺序、时长、具体画面、页面状态、素材来源、字幕、转场和 SFX；
只有完整分镜确认后才进入制作。用户从 Gallery 复制镜头名时，只需提供卡名或
“卡名 · 样式名”，Agent 负责读取本地配方卡和 demo 源码，不要求用户解释动画。
...
7. **廉价确认物前置：产品简报→需求到执行决策表→文字方向初选→styleframe→分镜逐级确认，
   方向问题不进逐镜头阶段。** 先用文字方向缩小创意范围，再用一个纯 HTML/CSS
   对比页 + 一张截图锁定全片色板/字体/材质/光感/构图；分镜表与设计 spec 同
   文档一次确认。确认物越廉价、越早，返工越便宜。

8. **验收贯穿全程 + 交付前独立审查。** 阶段 4 起每个镜头用
   `npx remotion still` 出静帧自检、每轮修改后整片渲染 + ffmpeg
   抽帧回看；交付前必须派一个干净上下文的 subagent 做独立终检。审查输入
   包含成片、关键帧、产品简报、需求到执行决策表、用户确认的视觉方向/styleframe、功能到镜头映射、
...
按 `references/pipeline.md` 执行制作。自由创作若已经通过引导流程确认产品简报、
路线、需求到执行决策表、视觉方向、镜头映射和分镜，不得回到阶段 0/2/3 重复询问；只补仍未完成的
确认物，然后进入素材采集、帧级时间轴、实现、声音与验收。阶段 3（分镜）时扫
...
| 时机 | 读 |
|------|----|
| 项目启动 | pipeline.md（只处理尚未确定的开工项；有项目且路线未定时先只读检查再推荐） |
| 自由创作路线 | guided-free-creation.md（产品简报、需求到执行决策表、视觉方向、镜头映射和分镜确认） |
