...
description: 用镜头配方卡 + 已验收模板 + 代码/音频资产制作电影感产品视频（Remotion + 真实页面截图 + 2.5D 运镜 + 节奏卡点 + 声音设计）。当用户要求"用 video-shotcraft 做视频/宣传片"、点名 Morning Paper 模板或要求复刻模板片效果，或要用镜头卡做单个动效镜头时使用。
---
...
- 当前可直接使用 `template/` 中一支**已验收的 36.2 秒产品宣传片模板
  Morning Paper（晨读）**：1920×1080、30fps、纸墨琥珀风格，含 10 个完整镜头、
...
**例外一：用户已点名 Morning Paper 模板时，视为模板路线已选定。**
例如"用 Morning Paper 模板给我的产品做宣传片"。此时不要再询问，
