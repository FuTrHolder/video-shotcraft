...
选 Ink Press 不等于跳过产品判断。先做最小只读检查，并产出一份精简的产品简报与
需求到执行决策表：写清产品定位、受众、用途、核心卖点、必须展示功能、可用页面
状态、数据口径，以及每项对模板替换的影响和依据。只追问无法从项目或用户材料
确定、且会影响成片的 1–3 项；不得把受众、用途或品牌直接套成固定风格。

然后生成**模板适配 spec**，在替换任何镜头前由用户确认：

```text
| 模板镜头 | 保留的镜头结构/运动语法 | 目标产品功能与页面状态 | 替换素材与文案 | 品牌 token 适配 | 数据处理 | 适配风险 |
|---|---|---|---|---|---|---|
```

Ink Press 只继承镜头结构、运动语法、节奏和已调参数；字体、色板、材质、文案和
页面素材必须按目标产品重新蒙皮。模板的纸墨琥珀气质若不适合当前产品，应在路线
判断阶段改走自由创作，而不是强行套用。模板适配 spec 与最终分镜共同构成制作放行。

数据按风险处理：公开演示数据仅在产品简报明确确认后可保留；客户、个人、内部、
密钥、实时或其他敏感数据必须先虚构、脱敏或冻结。用户尚未说明音频方向时，先问
是否保留 SFX-only、选 BGM、需要配音，或授权 Agent 推荐；不要默认把 SFX-only
当作最终音频选择。
...
1. **采集最终素材**（在模板适配 spec 与最终分镜放行后；详细流程见
   ../references/pipeline.md 阶段 1）：
...
5. **验收**：每改完一个镜头 `npx remotion still` 出静帧自检（每镜头至少
   入场中/动作峰值/落定后三帧）；全改完整片渲染 + ffmpeg 抽帧回看；最后必须
   按 ../references/final-review.md 由未参与制作的独立审查 Agent 对照产品简报、
   需求到执行决策表、模板适配 spec、最终分镜、音频选择与数据口径出报告，并对照
   ../references/aesthetic-rules.md 过 checklist。发现偏差先修复再终渲交付。
...
- 终检必须验证：模板镜头是否映射到已确认的目标功能与页面状态、替换文案是否符合
  核心卖点和受众、音频选择是否已落实、数据处理是否符合风险口径；不能只检查画面好不好看
